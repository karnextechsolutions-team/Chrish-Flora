...
export default function POSClient({ initialProducts }: Props) {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row overflow-hidden bg-gray-50 font-sans">
      ...
      {showReceipt && createdOrder && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            {/* Print, WhatsApp Share, & New Sale Buttons */}
              ...
              <button
                onClick={() => {
                  const message = generateReceiptMessage({
                    ...
                    dateTime: new Date(createdOrder.created_at).toLocaleString('en-LK', {
                      day: '2-digit',
                      month: '2-digit', 
                      year: 'numeric',
                      hour: '2-digit',
                  ...
                  const phone = createdOrder.customer_phone;
                  
                  if (!phone || phone.trim() === '' || phone === 'N/A') {
                    if (navigator.clipboard) {
                      navigator.clipboard.writeText(message).then(() => {
                        setToastMessage('Receipt copied! Paste in WhatsApp.');
                        setTimeout(() => setToastMessage(''), 3000);
                      });
                    } else {
                      alert('No phone number! Failed to copy receipt to clipboard.');
                    }
                    return;
                  }
                  
                  shareOnWhatsApp(phone, message);
                }}
                className="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1DA855] text-white font-semibold py-2.5 rounded text-xs shadow-[0_4px_12px_rgba(37,211,102,0.2)] hover:shadow-[0_6px_16px_rgba(37,211,102,0.35)] transition-all duration-200"
              >
                <svg 
      ...
      {toastMessage && (
        <div className="fixed bottom-20 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs px-4 py-2.5 rounded-full shadow-lg z-[100] animate-fade-in-up font-sans font-medium flex items-center gap-1.5 border border-white/10">
